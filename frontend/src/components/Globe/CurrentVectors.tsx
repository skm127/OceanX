...
function StreamingCurrents({ vectors, speedMin, speedMax }: { vectors: CurrentVector[]; speedMin: number; speedMax: number }) {
  ...
  const { positions, colors, speeds, latLonData } = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const pos = latLonToVector3(vec.lat, vec.lon, GLOBE_RADIUS + 0.015);
      positions[i * 3] = pos.x;
  ...
  useFrame((_, delta) => {
    ...
    if (particlesRef.current) {
      ...
      for (let i = 0; i < latLonData.length; i++) {
        ...
        // Update 3D position
        const newPos = latLonToVector3(newLat, newLon, GLOBE_RADIUS + 0.015);
...
function CurrentArrows({ vectors, speedMin, speedMax }: { vectors: CurrentVector[]; speedMin: number; speedMax: number }) {
  ...
  const geometry = useMemo(() => {
    const geo = new THREE.ConeGeometry(0.006, 0.03, 6);
  ...
  useEffect(() => {
    ...
    vectors.forEach((vec, i) => {
      const pos = latLonToVector3(vec.lat, vec.lon, GLOBE_RADIUS + 0.012);
      dummy.position.copy(pos);

      const posE = latLonToVector3(vec.lat, vec.lon + 0.5, GLOBE_RADIUS + 0.012);
      const posN = latLonToVector3(vec.lat + 0.5, vec.lon, GLOBE_RADIUS + 0.012);
      ...
      const scale = 0.4 + (vec.speed / (speedMax || 0.1)) * 1.5;
  ...
  useFrame((_, delta) => {
    ...
    if (meshRef.current) {
      ...
      material.opacity = 0.6 + 0.2 * Math.sin(timeRef.current * 2);
    }
  });
...
export default function CurrentVectors({ vectors, speedMin, speedMax, visible = true }: CurrentVectorsProps) {
  ...
  // Use streaming particles for high-density areas, arrows for lower density
  const useStreaming = vectors.length > 50;
