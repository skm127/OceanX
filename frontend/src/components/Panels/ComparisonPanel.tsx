...
export default function ComparisonPanel({
  ...
  const isCriticalAnomaly = anomalyAnalysis?.status === 'CRITICAL_ANOMALY' || anomalyCount >= 8;
  const isWarningAnomaly = anomalyAnalysis?.status === 'WARNING' || (anomalyCount >= 3 && !isCriticalAnomaly);

  // Export handlers
  const exportJSON = () => {
    const report = {
      title: 'OCEAN-X Model vs In-Situ Observation Intelligence Dossier',
    ...
    >
        ...
        <div className={`anomaly-hero-banner ${isCriticalAnomaly ? 'critical' : isWarningAnomaly ? 'warning' : 'nominal'}`}>
          ...
          {isCriticalAnomaly && (
            <div className="anomaly-advisory-box critical-advisory">
              ...
              <div className="advisory-item">
                <span className="advisory-dot">●</span>
                <span><strong>INCOIS Action:</strong> Assimilate Float #{profile?.platform_id} soundings into the 6-hr cycle to correct NEMO mixed-layer physics.</span>
              </div>
            </div>
          )}
          {isWarningAnomaly && (
      ...
      {loading && (
        <div className="panel-loading">
          ...
          Cross-matching vertical soundings against NEMO reanalysis...
        </div>
      )}
