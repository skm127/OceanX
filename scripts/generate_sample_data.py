...
def generate_model_data():
    ...
    # Time: 7 daily steps starting from a reference date
    ...
    for t in range(n_time):
        for d in range(n_depth):
            ...
            thetao[t, d] = base_temp * lat_factor * lon_factor * time_factor + noise
    ...
    for t in range(n_time):
        for d in range(n_depth):
            ...
            so[t, d] = base_sal * coast_factor * lat_effect + noise
    ...
    for t in range(n_time):
        for d in range(n_depth):
            ...
            # Add noise
            uo[t, d] += np.random.normal(0, 0.01, (n_lat, n_lon)).astype(np.float32)
            vo[t, d] += np.random.normal(0, 0.01, (n_lat, n_lon)).astype(np.float32)
    ...
    print(f"  ✅ Temperature range: {float(verify_ds['thetao'].min()):.1f}°C to {float(verify_ds['thetao'].max()):.1f}°C")
...
def generate_argo_data():
    ...
    verify_ds = xr.open_dataset(filepath)
    print(f"  ✅ Saved to {filepath}")
    print(f"  ✅ Profiles: {verify_ds.dims['N_PROF']}")
    print(f"  ✅ Depth levels per profile: {verify_ds.dims['N_LEVELS']}")
    print(f"  ✅ Lat range: {float(verify_ds['LATITUDE'].min()):.2f} to {float(verify_ds['LATITUDE'].max()):.2f}")
    print(f"  ✅ Lon range: {float(verify_ds['LONGITUDE'].min()):.2f} to {float(verify_ds['LONGITUDE'].max()):.2f}")
    print(f"  ✅ Temp range: {float(verify_ds['TEMP'].min()):.1f}°C to {float(verify_ds['TEMP'].max()):.1f}°C")
...
def main():
    ...
    print("=" * 60)
    print()
    print("⚠️  This generates SYNTHETIC data for development.")
